...
/**
 * BUNDLER RECONCILIATION (the same surprise decision-engine, shadow-run, and
 * memory-ledger already hit and documented — carried over verbatim as
 * shared build tooling, per this project's own house rule that infra is not
 * conceptual code):
 *
 ...
 * (e.g. lib/contracts/index.ts importing "./intervention.js", which is
 * really intervention.ts — the standard TypeScript `moduleResolution:
 * "bundler"` idiom, understood by tsc and by esbuild/Vite, which is why
 * `npm test` (vitest, esbuild-powered) resolves it fine).
 *
